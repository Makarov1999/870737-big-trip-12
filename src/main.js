...
const createInfoAndCostTemplate = () => {
  return (
...
renderElement(mainTripBlock, createInfoAndCostTemplate(), `afterbegin`);
renderElement(mainTripFirstCaption, createMainMenuTemplate(), `afterend`);
renderElement(mainTripFilterContainer, createFiltersTemplate(), `beforeend`);
renderElement(eventsTripCapture, createSortTemplate(), `afterend`);
renderElement(eventsTripContainer, createFormMarksRoute(), `beforeend`);
renderElement(eventsTripContainer, createDayListTemplate(), `beforeend`);
const daysList = eventsTripContainer.querySelector(`.trip-days`);
renderElement(daysList, createDayTemplate(), 'beforeend');
const dayEvents = daysList.querySelector(`.day`);
renderElement(dayEvents, createEventListTemplate(), `beforeend`);
const eventsList = dayEvents.querySelector(`.trip-events__list`);
for (let i = 0; i < EVENTS_COUNT; i++) {
  eventsResultTemplate+=createRoutePointTemplate();
}

renderElement(eventsList, eventsResultTemplate, `beforeend`);
